...
- `docs/`：存放架构、差距分析和路线图等长期设计文档，例如 runtime architecture、gap analysis、vendor enhancement roadmap。

## 文档目录约定

- `knowledge-base/` 用于记录用户反馈的问题模式和已验证修复方案；新增问题先写入 `question-cases.md`，稳定根因和修复落点再沉淀到 `fix-playbook.md`。
- `fixtures/` 用于记录回归样本、页面索引、问题标签和基线；新增样本优先补用途、重点页面、标签和状态，不要复制真实 PPTX 二进制文件，真实样本继续引用 `public/` 中的文件。
